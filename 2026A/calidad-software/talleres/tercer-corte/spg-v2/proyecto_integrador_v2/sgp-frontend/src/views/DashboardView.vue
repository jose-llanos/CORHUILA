<template>
  <div>
    ...
    <template v-else>
      <v-alert v-if="estaPenalizado" type="error" variant="tonal" border="start" prominent class="mb-5">
      ...
      <v-row>
        <v-col v-for="action in clienteActions" :key="action.to" cols="12" sm="4">
          <v-card :to="action.to" color="surface" class="action-card pa-5" :class="`action-card--${action.color}`" hover>
...
onMounted(async () => {
  if (isAdmin.value) {
    const [usuarios, equipos, prestamos, penalizaciones] = await Promise.allSettled([
      usuarioService.list(),
      equipoService.list(),
      prestamoService.list(),
      penalizacionService.list(),
    ])
    ...
    if (prestamos.status === 'fulfilled') {
      const misPrestamos = prestamos.value.data.filter(p => p.usuario?.id === uid)
    ...
    if (penalizaciones.status === 'fulfilled') {
      const misPen = penalizaciones.value.data.filter(p => p.usuario?.id === uid)
