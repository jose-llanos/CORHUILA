      ...
      <v-row dense>
        ...
        <v-col cols="12">
          <v-text-field
            ...
            :hint="selected ? 'Dejar vacío para no cambiarla' : 'Mínimo 6 caracteres'"
            persistent-hint
            type="password"
...
const form = reactive({ nombre: '', email: '', password: '', rol: 'CLIENTE', estado: 'ACTIVO' })
...
function openCreate() {
  isEdit.value = false
  Object.assign(form, { nombre: '', email: '', password: '', rol: 'CLIENTE', estado: 'ACTIVO' })
  dialog.value = true
}
...
function openEdit(item) {
  isEdit.value = true
  selected.value = item
  Object.assign(form, { nombre: item.nombre, email: item.email, password: '', rol: item.rol, estado: item.estado })
  dialog.value = true
}
...
async function handleSave() {
  ...
  try {
    ...
    if (isEdit.value) {
      await usuarioService.update(selected.value.id, payload)
...
async function handleDelete() {
  saving.value = true
  try {
    await usuarioService.remove(selected.value.id)
