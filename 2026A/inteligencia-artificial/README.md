# 🤖 Bienvenidos a la Asignatura de  
## INTELIGENCIA ARTIFICIAL [G1]-82756-90B-2026A  

### 👥 Grupo 4

Bienvenidos al repositorio del **Grupo 4**, donde podrás encontrar todos los **talleres, actividades y trabajos** correspondientes a los diferentes cortes de la asignatura.

---

## 📂 Contenido del Curso

Haz clic en cada sección para acceder directamente:

- 📌 [📁 Primer Corte](./talleres/primer-corte)  
- 📌 [📁 Segundo Corte](./talleres/segundo-corte)  
- 📌 [📁 Tercer Corte](./talleres/tercer-corte)   

> 🔗 *Los enlaces serán actualizados a medida que se suban los trabajos.*

---

## 👨‍💻 Integrantes del Grupo 4 (Orden Alfabético)

- 👤 **JUAN FERNANDO PEREZ OLAYA**  
- 👤 **NELSON DAVID GUTIERREZ MEDINA**  
- 👤 **OSCAR FABIAN LEON TRIANA**  
- 👤 **SNEIDER MONROY QUIROGA**  
- 👤 **WILKYN JULIAN VARGAS BAHAMON**  

---

## 🎯 Objetivo del Repositorio

Este repositorio tiene como propósito:

- 📘 Centralizar los trabajos de la asignatura  
- 🧠 Aplicar conceptos de Inteligencia Artificial  
- 💻 Desarrollar soluciones prácticas  
- 📊 Documentar cada avance por corte académico  

---

## 🚀 Tecnologías Utilizadas

- 🐍 Python  
- 📊 Jupyter Notebook  
- 🤖 Machine Learning  
- 🧠 Deep Learning  

---

## 📌 Notas

- Cada corte contiene sus respectivas actividades organizadas en carpetas.
- Se recomienda revisar constantemente los avances del repositorio.

---

## ⭐ ¡Gracias por visitar nuestro repositorio!
